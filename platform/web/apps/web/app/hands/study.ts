 ...
 * poker_tenant_1", the per-account budget of plan E.3), which reaches the browser as a 500 the API
 * has sanitized down to "Internal server error". Nothing is wrong with the hand and nothing is
 * missing from the pool: the question is simply worth asking again, and these sentences say so.
 *
...
/**
 * Why the answer is missing, in one clause. A 5xx the API would not describe is the refusal above;
 * the ones it does describe — a query over the account's budget, a timeout — keep the server's own
 * sentence, which says something this one cannot.
 *
 * `describeApiError` now reads the same sanitized 5xx the same way, so the two agree; this copy
 * stays because its wording names the thing that asked too much — stepping through a hand — and
 * the app-wide one cannot.
 */
function reason(error: unknown): string {
  const status = errorStatus(error);
  const detail = errorDetail(error);
  if (status !== undefined && status >= 500 && (detail === null || detail === SANITIZED)) return UNDER_LOAD;
