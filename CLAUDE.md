...
- Layout: `parser/sites/` (one file or package per network) · `ingestion/` (upload → MinIO →
  ...
  9-step analyzer (spec §15) is `apps/web/app/analyze/` (the step definitions as data, the spot
  the earlier steps build, the reveals, the Dexie autosave) over `/v1/analyses`, whose saves
  **merge by step number** so a partial autosave cannot lose earlier work (ADR-034). The
