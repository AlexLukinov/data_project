...
**Previously:** 2026-09-12 (session 18, **the round-3 merge** — four parallel lanes (E.3, E.1, D.6, D.4) reconciled and committed as `8912598`, `15b39c4`, `c26ab01`, `fb9b0c7`. **E.3 ticked**; E.1 deliberately left `[ ]` pending E.1b. `make check` green (1,552) · `make web-check` green (806 tests / 76 files) · `make seed && make test-all` **1,655 passed, 6 skipped** — green only after two fixes to E.3, the lane whose gate had never been run.)
...
### ✅ F.12a — the audit's mechanical half: done 2026-09-15 (round 5, lane C, ADR-053) — committed `a60611e`
...
### ✅ D.9a — retire the v1 API: ticked at the merge 2026-09-15 (round 5, lane B, ADR-052) — committed `dd770bd`
...
`make seed` + `make test-all`. At the founder's request, lane D's code was committed alone (`5c3b55f`)
and **`feat/range-lab` was pushed** (the repository is public). The first run was red at `make up`, on
the one-shot `minio-init` under `docker compose up --wait`. `88625fd` fixed it, and
...
**nothing after `88625fd` is pushed**, and the next push waits for the founder's decision on the public
...
| commit | step |
|---|---|
| `5c3b55f`, `88625fd` | **F.1** CI at the repository root, calling the make targets (ADR-054) — committed and pushed by its lane at the founder's request; green in run 34957886151 |
| `dd770bd` | **D.9a** retire the v1 API; the winnings curve on `/v1/hero/winnings` (ADR-052) — **ticked on merge** |
| `b1a535a` | **D.8** upload & accounts: `/upload` → My game with no command (ADR-051) — ticked by its lane |
| `a60611e` | **F.12a** one number input for `2,5` and `2.5`; the audit's mechanical fixes (ADR-053) — ticked by its lane |
| `c36faa4` | fix: the heuristic log stamps a review with the database's clock (ADR-038's correction) |
...
| commit | step |
|---|---|
| `555c20d` | **E.1b** the hot path — an upload is in stats with no dbt run (ADR-047) — **ticked, and E.1 with it** |
| `0ec8ca5` | **D.7b** notes and tags on a hand, `?tag=` on every hand list (ADR-048) — **ticked on merge** |
| `4637393` | **D.6b** cohort create / edit / copy / delete (ADR-049) — ticked by its lane |
| `7065194` | the **F.12 audit** — [POKER_UX_AUDIT.md](POKER_UX_AUDIT.md); F.12 itself unstarted |
...
**Where things stand (2026-09-10):** phases A–C are done and merged into `main` (`7b6eb5f`);
...
**Where F.6 stands (2026-09-10):** committed as `3bd5f46` (ADR-031). **`NodeKey`
...
**Where F.7 stands (2026-09-10):** committed as `94bdb90` (ADR-032). The replay
...
**Where F.5 stands (2026-09-10):** committed as `d77a76f`.

**Where D.2 stands (2026-09-10):** committed as `ca3f3b6`. `apps/web/app/auth/{api,session,
...
first UI is running. F.1 (`021e9c7`), F.2 (`075b498`), F.3 (`ea00ae4`) and F.4 = D.1
(`ffa496b`) are committed. F.4: `platform/web/apps/web` (Nuxt 4 SPA; `make web` on :3000;
...
4. **D.3 is done and verified, uncommitted** — commit it when the founder says so, separately from
   ...
   hit and solved).
5. **F.10 is committed** (`57b3b9a`) — the paragraphs above describing it as uncommitted are
...
| 2026-09-09 (11) | C.5 merged (`419b2fd`). **C.6 parity done**: `scripts/fingerprint.py` → `reports/parity_2026-09-09.md`, 756 cells, 0 mismatches, 20 more registry `notes`; two v1 undercounts found (4-bet%, 5-bet%). Cut-over drops await confirmation. | Plan **C.6 cut-over** (after the founder confirms the drops) |
| 2026-09-09 (10) | C.4 merged (`2359d42`). **Plan C.5 done**: `/v1/definitions`, `/v1/reports/run`, saved filters/reports/stats CRUD (migration `513730dcd5be`), v1 `/v1/stats*` routes as adapters over the engine. 12 unit + 3 integration tests; `make check` 317, `make test-all` 331 + 2 skipped. | Plan **C.6** (parity report, then the v1 cut-over) |
| 2026-09-09 (9) | C.3 merged (`3d939d9`). **Plan C.4 done**: `stats.service.run_report` — resolve, route (rollup vs facts by stats and dimensions), one bound-parameter query per plan, merge, population baseline, per-tenant cache; 37 new unit tests (305); real reports verified from all three tables. | Plan **C.5** (API v2 + saved filters/reports/stats) |
| 2026-09-09 (8) | C.1 + C.2 committed and merged into `main` (`49849c7`). **Plan C.3 done**: `scripts/gen_stats.py` renders the rollup (`marts.stats_daily_v2`), the definitions seed and the law test from the registry; `stats/compiler.py` underneath; `make gen`/`gen-check`/CI extended. Rollup bootstrapped in 35 passes / 186 s at ≤1.66 GiB; sums equal the facts and v1; law test green on real data. `make check` 273. | Plan **C.4** (router + service on the compiler) |
...
| Generator | `platform/scripts/pool_report.py` at commit `419b2fd` — deleted with the v1 chain (plan C.6); the same figures come from `POST /v1/reports/run` on `population` |
