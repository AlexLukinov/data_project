...
**Current phase: phases A–F complete; phases G and H under way** — round 9 merged 2026-09-23 (G.1–G.4, H.0–H.3 and H.7a ticked; open G.5, H.4–H.6, H.7b, H.8, H.9) — 83 steps, **76 ticked** · **Status: 9.1M real hands loaded · the real marts carry `flop_connectivity`, `flop_high_card_class`, `turn_change` and `river_change` · `make check` 1,844 · `make web-check` 1,791 / 163 · history rewritten, the fifteenth handle gone**
**Last updated:** 2026-09-23 (session 28, **the round-9 merge and the second history rewrite** — four lanes committed (texture `281b748` · key `ad82b26` · inference `c93c141` · replayer `bf9932b`) and the docs; G.1–G.4, H.0–H.3 and H.7a ticked; sixteen ADRs, 081–096. Gates over the combined tree: `make check` **1,844** · `make web-check` **1,791 / 163** · `make seed && make test-all` **1,964 passed, 6 skipped** · `make e2e` **2 / 2** · `make privacy-check` clean. Then the fifteenth real handle was rewritten out of every commit on every branch — rehearsed in a mirror clone, 92 commits, only the two parser files that held it changed, a full-history privacy scan clean — and `feat/range-lab` force-pushed with a lease.)

**Previously:** 2026-09-23 (session 27, **the push, and D.9b ticked** — the G/H planning docs committed as `d5d2a2a` with the six ADR index rows they lacked and H.10 ticked; pushed as a fast-forward with the privacy hook clean; **the e2e job's first CI run was red** on a spec selector that F.13's explainer made ambiguous, fixed in `7fac39b` from a clean clone; **run 35833221826 green on all four jobs**. Phases A–F are complete. Round 9's four lanes are running.)
...
**Previously:** 2026-09-22 (session 24, **the round-8 merge** — four workstreams committed: B.5b `d434fd1`, the registry lane `b1da490`, F.14 `1241339`, the leftovers lane `fcb16fe`, plus the docs. **F.14 ticked**, so every plan step but D.9b's is closed. Eleven ADRs, 064–074, and for the first round no merge fixes were needed at all. `make check` **1,756** · `make web-check` **1,656 / 156** · `make seed && make test-all` ****1,869 passed, 6 skipped**** · `make privacy-check` clean. Still unpushed, 25 commits ahead.)
...
**Previously:** 2026-09-21 (session 22, **the round-7 merge** — three lanes committed (`33464fd`, `e253b08`, `a896acb`) plus the platform scripts (`7337842`); **F.12 ticked** and **F.14 added**. `make check` **1,735** · `make web-check` **1,601 / 152** · `make seed && make test-all` **1,847 passed, 6 skipped** · `make privacy-check` clean.)
...
**Previously:** 2026-09-15 (session 20, **the round-5 merge** — D.9a `64f1947`, D.8 `60a31f7`, F.12a `250c189` committed, plus a heuristics clock fix `817afa8` and a fixture scrub (later folded into the history rewrite); F.1 had committed and pushed itself. **D.9a ticked.** Gates green on the third run: `make check` 1,675 · `make web-check` 1,015 / 99 · `make seed && make test-all` **1,783 passed, 6 skipped**. **Found: fourteen real opponents' screen names in the public history** — the founder's decision, below.)

**Previously:** 2026-09-15 (session 19, **the round-4 merge** — E.1b `c7b3be9`, D.7b `5548c96`, D.6b `01441d1` and the F.12 audit `93f115b` reconciled and committed. **D.7b ticked** after its six never-run integration tests passed by name; E.1 and E.1b ticked by their lane. `make check` green · `make web-check` green (846 / 79 files) · `make seed && make test-all` **1,715 passed, 6 skipped**. Found: the real Postgres is four create-only migrations behind head.)
...
**Previously:** 2026-09-12 (session 18, **the round-3 merge** — four parallel lanes (E.3, E.1, D.6, D.4) reconciled and committed as `2df725a`, `742d5b3`, `2bd33b5`, `50b0fd5`. **E.3 ticked**; E.1 deliberately left `[ ]` pending E.1b. `make check` green (1,552) · `make web-check` green (806 tests / 76 files) · `make seed && make test-all` **1,655 passed, 6 skipped** — green only after two fixes to E.3, the lane whose gate had never been run.)
...
### ▶ Next: **round 10** — the cube, the collapse and the exploits (H.4, H.5, H.6), then the replayer's last panels

Round 9 closed **G.1–G.4, H.0–H.3 and H.7a** (ADR-081…096): the board means one thing in SQL, the
registry, the filter and TypeScript; a situation is a line across streets; a pool frequency carries
its cluster-robust interval and every player has a group; and the replayer shows the pool's range
beside your read, on a press. What phase H still needs, in dependency order:

1. **H.4 `marts.node_ranges`** (ADR-079). Every key field it needs now exists: `line_so_far`,
   `pot_type`, `size_bucket` (ADR-086/087), the board hierarchy `flop class → turn_change →
   river_change` on real columns (ADR-082/084), and ADR-080's five groups addressable as
   `group:<key>` (ADR-092). **A `NodeKey` cannot say heads-up** (`players_live` is not a key field),
   so the cube's HU scope is its build filter, not the key's. Bootstrap with `scripts.backfill`, never
   `--full-refresh` (ADR-019); the real rebuild is exclusive, as ADR-084's was.
2. **H.5** — cube-vs-fact routing and the adaptive collapse, on G.3's interval gate (ADR-090);
   `textureTags` emits the hierarchy in collapse order, so collapsing is dropping tags from the end
   (ADR-083). When it lands the replayer's reveal answers from the cube with no client change.
3. **H.6** — `analysis/exploits/`, interval-gated; its Done means reproduces session 25's numbers
   from the cube.
4. **H.7b · H.8 · H.9** — the exploit panel, the per-seat group badge (`cohorts[].group` from
   `/v1/pool/presets`), the player panel. **G.5** is one preset, now unblocked.

**Follow-ups the round-9 lanes wrote down**, each with its file:
- the replayer passes no `facing_size_pct` buckets to `nodeKeyAt` (`hands/panels.ts:79-80`, a prop on
  `packages/poker-ui/src/components/HandReplayer.vue`), so a replayer node has no `size_bucket` yet;
- the range library's lookup is exact equality over the whole key (`api/range_library.py:81`,
  `ranges/library.ts:154`), so a postflop replayer key — now carrying line, pot type and texture —
  finds an authored chart only if it was saved from the replayer's own key; the key lane proposes
  that a stored chart's unset fields mean "any" (ADR-086), which needs a decision of its own;
- `between` is inclusive at both ends while node buckets are half-open (`stats/compiler.py:134`):
  46 of 48,914 barrels sit on a stack edge and belong to two nodes;
- one frequency, two intervals: `/pool`'s presets run un-clustered while the replayer's node answers
  are clustered, and the client's `ReportRequest` cannot ask for `cluster` (`stats/api.ts`);
- `PoolDataBadge` shows no `players`; `HandStudy.vue` is 366 lines; `NodeKeyEditor` has no control
  for the three new key fields; `nodeKeyLabel` does not mention the line; a `facing_raise_to_bb`
  column would make a preflop size filterable; `/v1/hero/leaks` still takes `cohort_id` only;
- at a fresh step a reader who presses both panels can reach ~10 queries — a worded 429, not a 500,
  until the cube.

**Only the founder can finish the privacy clean-up** — GitHub keeps what a rewrite cannot reach:
- **Delete six workflow runs** that link pre-rewrite commits: `34955622749`, `34957886151`, `34991634020`, `35830927100`, `35833221826`, `35834121430` (Actions → the run → ⋯ →
  Delete workflow run). Keep the run on the rewritten tip.
- **Ask GitHub Support to purge the unreferenced commits and cached views**
  ([support.github.com](https://support.github.com/request), "Remove data from a repository"), naming
  the repository and both pre-rewrite tips: `2457c344b936f36166f16aeecc88e7d059e1c391` (2026-09-23)
  and `3a4b2ea271ca35675a663ade60b0b2a894d67520` (2026-09-15). Until they confirm, making the
  repository private hides the old SHAs too.
- `platform/backups/ru_de-all-refs-pre-scrub-20260915.bundle` and `…-pre-scrub2-20260923.bundle` still
  hold the names: local and gitignored; delete them once nothing needs them.
- Still yours too: when to `UNFREEZE` B.5b's `b5b_20260921` snapshot (9.1 GB), and the nine-label
  voice proposal.
...
- **Texture** (G.1, G.2, H.2, H.3; ADR-081…084). `flop_connectedness` — wrong on 73.1% of real
  flops — is retired for `flop_connectivity`; `flop_high_card_class` beside the raw rank; `turn_change`
  and `river_change` name what each later card did, by a stated precedence, with the street in every
  value because a tag is resolved by value. Every classification is a macro, and the 38-board fixture
  is run through the dbt SQL itself and through `textureTags()`. **The one write to the real
  ClickHouse this round**: three ALTERs proven identical to the test-built tables, then a forced
  backfill — 38 passes, 21 minutes, row counts and the hero fingerprint unmoved, `make dbt-test`
  29/29.
- **Key** (H.1; ADR-085…088). `NodeKey` carries `line_so_far`, `size_bucket` and `pot_type`; a key
  has one account of a street; the replayer keys a node from the whole hand, with texture. **Stored
  keys were migrated** (`b9c0d1e2f3a4`) — the plan had not mentioned that three new fields would make
  every saved range miss its own node. The `r/b/b/` barrel node answers on the real data.
- **Inference** (G.3, G.4; ADR-089…092). A pool frequency carries a cluster-robust interval computed
  over players in one two-level query, and `min_n` means what this node needs — 1,801 on ADR-076's
  node, reproduced to the digit (SE 0.1429pp, design effect 4.586). Seven cohorts partition the pool
  (94,276 of 94,276), five groups are unions of them, and either is one `?cohort=` away without saving.
  ADR-076's "ICC ≈ 0.24" is ≈0.017 (erratum placed).
- **Replayer** (H.0, H.7a; ADR-093…096). ClickHouse 202 is a worded 429, proven live with an 8-query
  burst. The reveal: paint your read, then on a press the pool's range for the field and one group,
  diffed; the blocker table cut at the pool's measured fold rate by equity. A per-class tier-3 split
  was built and removed the same day because with no folds revealed it ranks nothing (ADR-094).
  `BlockerPanel` rows are controls only when selectable; a Worker that cannot start rejects.

**The fifteenth handle, rewritten out of every commit.** `git filter-repo --replace-text` with two
literal rules (the comment line as round 6 rewrote it, and the name alone), rehearsed first in a
mirror clone and repeated in place with the same 110 hashes: 92 commits rewritten, only
`parser/sites/pokerstars.py` and `parser/sites/pokerstars/grammar.py` changed in content, the tip's
tree byte-identical, and a full-history privacy scan of every branch clean — where the same scan over
the old history finds 3 places. A bundle of every ref was written first. The 212 commit hashes these
docs cite were remapped to their rewritten counterparts. GitHub still serves the old SHAs until
Support purges them; the runs to delete and the request are in Next action.

---
### ✅ Round 8 is merged (2026-09-22) — four workstreams, **F.14 ticked, the plan closed**

**Committed:** B.5b `d434fd1` · the registry lane `b1da490` · F.14 `1241339` · the leftovers lane
`fcb16fe`. **Nothing is pushed.**
...
**Committed:** lane A (F.12's close-out) `33464fd` · lane B (the analyzer) `e253b08` · lane C (the
pool's lookup and the registry) `a896acb` · the platform scripts `7337842`. **Nothing is pushed.**
...
- **`make start` / `make pause` / `make stop`** (`7337842`) — the whole platform in one command
...
**Committed:** D.9b `aa9ec21` · D.9c `4e4b245` · F.12b `7504651` · F.12c `90e5419` · F.12d `9d576a5` ·
F.13 `eac820f` · the allowlist entry `d876733`. Before them, the unpushed docs commit was amended
(`b2213ed` → `d7ca132`) to fold out a name it quoted. **Nothing is pushed.**
...
---
### ✅ D.8 — upload & accounts: done 2026-09-15 (round 5, lane A, ADR-051) — committed `60a31f7`
...
### ✅ F.12a — the audit's mechanical half: done 2026-09-15 (round 5, lane C, ADR-053) — committed `250c189`
...
### ✅ D.9a — retire the v1 API: ticked at the merge 2026-09-15 (round 5, lane B, ADR-052) — committed `64f1947`
...
`make seed` + `make test-all`. At the founder's request, lane D's code was committed alone (`5d8327f`)
and **`feat/range-lab` was pushed** (the repository is public). The first run was red at `make up`, on
the one-shot `minio-init` under `docker compose up --wait`. `99c9a5b` fixed it, and
...
**nothing after `99c9a5b` is pushed**, and the next push waits for the founder's decision on the public
...
### ✅ The public history: fourteen names removed (2026-09-15), **the fifteenth on 2026-09-23**, and a guard so it cannot happen again

**Update, 2026-09-23 (the round-9 merge).** The fifteenth handle is gone from every commit on every local branch and from `origin/feat/range-lab` (the round-9 block above has the method and the proofs). What only the founder can do is now the GitHub side alone — six workflow runs to delete and a purge request — and the current list is in **Next action**; the bullets further down are the 2026-09-15 version of it.
...
regex is greedy. **The working tree now carried an invented name of the same shape, but the commit then public
(`88625fd`, before the second rewrite) and its ancestors still carried the real one.** Removing it means a second `git filter-repo --replace-text` and a
...
the rewrite — was folded out of the unpushed docs commit (`b2213ed` → `d7ca132`) before anything else was
committed.
...
handles**: one pool player's key with their WTSD and WWSF in PLAN and STATUS, a real regular's key in a web
test, two real hands with nine handles in the GGPoker parser test, and three handles in an old version of
`parser/sites/ggpoker.py`. None were ever on `origin/main`.

**What was done, at the founder's instruction.**
1. **Backup first:** `platform/backups/ru_de-all-refs-pre-scrub-20260915.bundle` — every ref as it was,
   verified, gitignored. **It still contains the names**; keep it local, and delete it once nothing needs it.
2. **A trial in a mirror clone**, proven before touching this repository: `git filter-repo --replace-text`
   with the fourteen names mapped to invented ones (the same ones the tree already used; the four-character
   one only when not inside a hex or base64 run, so no lockfile hash could be touched). Checked: `feat/range-lab`'s final tree
   **byte-identical** (`e3971eb…` before and after); for every one of the 66 rewritten commits, **only the five
   files that held a name changed**; the full-history scan — every blob on every ref, hand-history lines at
   any length — finds **none** of the fourteen; no commit message ever mentioned one.
3. **The same rewrite in place**, producing exactly the trial's hashes; the same scan, clean. All five local
   branches were rewritten (`main` included, which held the old parser test); the fixture-scrub commit became
   empty and was dropped. Every commit hash in these docs was remapped to its rewritten counterpart.
4. **Force-pushed with a lease** (`--force-with-lease` on the old tip): `origin/feat/range-lab` is now the
   rewritten counterpart of what was already public. **Round 5's merge stays local and unpushed.** CI started
   on the rewritten tip.

**Still needs the founder — GitHub keeps what a rewrite cannot reach.** The old commits remain viewable **by
SHA** (checked anonymously: the old tip answers 200), and two Actions runs link them.
- **Delete the two old workflow runs** — Actions → the runs on `feat/range-lab` from before the rewrite (the
  failed one and the green one; *not* the run on the rewritten tip) → ⋯ → Delete workflow run.
- **Ask GitHub Support to purge the unreferenced commits and cached views** —
  [support.github.com](https://support.github.com/request), "Remove data from a repository", naming the
  repository and the old tip `3a4b2ea271ca35675a663ade60b0b2a894d67520`; GitHub's guide "Removing sensitive
  data from a repository" is the procedure they follow.
- **Until Support confirms, making the repository private** hides the old SHAs too; it can go public again
  afterwards.
- Anyone who cloned or forked in those hours keeps a copy; nothing in git can recall that.

**For D.9c (running now):** its regression target `3a4b2ea` no longer exists in this repository. Clone the
bundle into the scratchpad (`git clone platform/backups/ru_de-all-refs-pre-scrub-20260915.bundle <scratch>`)
and scan that — never into the repository.

### Round 5 is merged (2026-09-15)

| commit | step |
|---|---|
| `5d8327f`, `99c9a5b` | **F.1** CI at the repository root, calling the make targets (ADR-054) — committed and pushed by its lane at the founder's request; green in run 34957886151 |
| `64f1947` | **D.9a** retire the v1 API; the winnings curve on `/v1/hero/winnings` (ADR-052) — **ticked on merge** |
| `60a31f7` | **D.8** upload & accounts: `/upload` → My game with no command (ADR-051) — ticked by its lane |
| `250c189` | **F.12a** one number input for `2,5` and `2.5`; the audit's mechanical fixes (ADR-053) — ticked by its lane |
| `817afa8` | fix: the heuristic log stamps a review with the database's clock (ADR-038's correction) |
| *(folded in)* | test: real screen names in two fixtures replaced with invented ones — empty after the history rewrite, which applied the same replacements to every commit, so it was dropped |
...
| commit | step |
|---|---|
| `c7b3be9` | **E.1b** the hot path — an upload is in stats with no dbt run (ADR-047) — **ticked, and E.1 with it** |
| `5548c96` | **D.7b** notes and tags on a hand, `?tag=` on every hand list (ADR-048) — **ticked on merge** |
| `01441d1` | **D.6b** cohort create / edit / copy / delete (ADR-049) — ticked by its lane |
| `93f115b` | the **F.12 audit** — [POKER_UX_AUDIT.md](POKER_UX_AUDIT.md); F.12 itself unstarted |
...
**Where things stand (2026-09-10):** phases A–C are done and merged into `main` (`76ab423`);
...
**Where F.6 stands (2026-09-10):** committed as `9595ec7` (ADR-031). **`NodeKey`
...
**Where F.7 stands (2026-09-10):** committed as `cdf5961` (ADR-032). The replay
...
**Where F.5 stands (2026-09-10):** committed as `6b55be6`.

**Where D.2 stands (2026-09-10):** committed as `d1a12ec`. `apps/web/app/auth/{api,session,
...
first UI is running. F.1 (`3b4cdee`), F.2 (`433b47b`), F.3 (`eb8b025`) and F.4 = D.1
(`a710e5c`) are committed. F.4: `platform/web/apps/web` (Nuxt 4 SPA; `make web` on :3000;
...
4. **D.3 is done and verified, uncommitted** — commit it when the founder says so, separately from
   ...
   hit and solved).
5. **F.10 is committed** (`50e2053`) — the paragraphs above describing it as uncommitted are
...
|---|---|---|
| 2026-09-23 (session 28 — **the round-9 merge and the second history rewrite**) | Four lanes committed (texture `281b748` · key `ad82b26` · inference `c93c141` · replayer `bf9932b`); **G.1–G.4, H.0–H.3 and H.7a ticked**, H.7 split, sixteen ADRs 081–096 placed. Gates re-run over the combined tree: `make check` **1,844** · `make web-check` **1,791 / 163** · `make seed && make test-all` **1,964 passed, 6 skipped** · `make e2e` **2 / 2** · `make privacy-check` clean. **The fifteenth handle rewritten out of every commit** (rehearsed, 92 commits, only the two parser files changed, full-history scan clean), the docs' 212 commit hashes remapped, the branch force-pushed with a lease. Round 10's prompts written. | **Round 10** (H.4, H.5, H.6); the founder deletes six Actions runs and asks GitHub Support to purge |
| 2026-09-23 (session 27 — **the push, and D.9b ticked**) | The G/H planning docs committed (`d5d2a2a`, six missing ADR index rows added, H.10 ticked) and pushed as a fast-forward, privacy hook clean. The e2e job's first CI run was red on a spec selector F.13's explainer made ambiguous; fixed in `7fac39b` from a clean clone, run 35833221826 green on all four jobs. **Phases A–F complete.** Round 9's prompts written after checking the new plan against the code. | The round-9 merge |
...
| 2026-09-09 (11) | C.5 merged (`7879395`). **C.6 parity done**: `scripts/fingerprint.py` → `reports/parity_2026-09-09.md`, 756 cells, 0 mismatches, 20 more registry `notes`; two v1 undercounts found (4-bet%, 5-bet%). Cut-over drops await confirmation. | Plan **C.6 cut-over** (after the founder confirms the drops) |
| 2026-09-09 (10) | C.4 merged (`a3dc43d`). **Plan C.5 done**: `/v1/definitions`, `/v1/reports/run`, saved filters/reports/stats CRUD (migration `513730dcd5be`), v1 `/v1/stats*` routes as adapters over the engine. 12 unit + 3 integration tests; `make check` 317, `make test-all` 331 + 2 skipped. | Plan **C.6** (parity report, then the v1 cut-over) |
| 2026-09-09 (9) | C.3 merged (`1aed2d9`). **Plan C.4 done**: `stats.service.run_report` — resolve, route (rollup vs facts by stats and dimensions), one bound-parameter query per plan, merge, population baseline, per-tenant cache; 37 new unit tests (305); real reports verified from all three tables. | Plan **C.5** (API v2 + saved filters/reports/stats) |
| 2026-09-09 (8) | C.1 + C.2 committed and merged into `main` (`5daac54`). **Plan C.3 done**: `scripts/gen_stats.py` renders the rollup (`marts.stats_daily_v2`), the definitions seed and the law test from the registry; `stats/compiler.py` underneath; `make gen`/`gen-check`/CI extended. Rollup bootstrapped in 35 passes / 186 s at ≤1.66 GiB; sums equal the facts and v1; law test green on real data. `make check` 273. | Plan **C.4** (router + service on the compiler) |
...
| Generator | `platform/scripts/pool_report.py` at commit `7879395` — deleted with the v1 chain (plan C.6); the same figures come from `POST /v1/reports/run` on `population` |
