...
| **Last updated** | 2026-09-11, session 10 (F.11: the six training modes, the scoring store, `/progress`, the heuristic log; ADR-038) |
| **Blockers** | none |
...
- [ ] **E.5** Equity (F-902/F-309) · vetted evaluator at parse time → `ev_won_bb` real, `made_hand` filled · **Done means** on a hand with a preflop all-in, `ev_won_bb != net_won_bb`; `made_hand` is non-empty on every decision where hole cards are known; the EV line on the chart separates from the actual line. *2026-09-11, code complete and `make check` green (1,524 unit tests); **stays `[ ]` until the corpus backfill and the mart rebuild are verified**.* **The licence gate passed without a new dependency:** `phevaluator` (Apache-2.0) — the Python binding of the same `HenryRLee/PokerHandEvaluator` the Range Lab already runs through WebAssembly (ADR-027) — was declared in `pyproject.toml` from the first platform commit and never imported. Nothing was ported from it; the **classifier** is a real port of `classify.ts` into `core/classify.py`, pinned to the shared fixture `tests/fixtures/made_hands.json` (1,024 cases, all 17 classes, generated from the TypeScript reference by `poker-core/fixtures/gen_made_hands.ts` and asserted by *both* suites — the ADR-028/031 pattern). New: `core/{cards,classify,equity,allin}.py`, `ingestion/pipeline.py` calls `enrich()` after validation so the worker and the bulk importer agree by construction, `ch/migrations/0010_made_hand.sql` (three `LowCardinality` columns on `core.hand_players`, one per street), `core/schema/players.py` + `make gen`, `macros/{hand_arrays,decision_state}.sql`, `made_hand` in `dimensions.yaml`, and `scripts/{backfill_equity,equity_store}.py`. **Equity is exact, never sampled:** 1,712,304 runouts for a heads-up preflop all-in at 1.25 s, made affordable because `canonical_key` folds suit relabelings and player order together — the corpus's 60,709 heads-up preflop all-ins reduce to **6,131** distinct match-ups. **Two decisions the data forced** (ADR-039): EV **redistributes the pot that was actually awarded, per side-pot layer**, rather than taking ADR-018's escape hatch of restricting to heads-up — which gives the invariant `sum(ev_won) == sum(net_won)` per hand exactly, so rake and drops need no special case; and there is **no adjustment where the runout never happened** — GGPoker settles an all-in on request without dealing the rest of the board, and **27,088 hands (20% of the pool's all-ins, 18 of hero's)** stop with exactly the cards betting stopped on. Two dbt assertions, both new: `assert_made_hand_is_set_exactly_where_the_cards_are` (unsampled — a streaming single-table scan — and scoped to hold'em) and `assert_ev_won_bb_redistributes_the_pot` (one hand in 64 **on `cityHash64(hand_uid)`**, never on the calendar). Hero backfilled in 46 s (10,525 seats, 522 → 486 equities after the cash-out rule was fixed); the pool backfill was running at the end of the session.
