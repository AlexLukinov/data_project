...
| | |
|---|---|
| **Phase** | **D — UI** (phase C complete 2026-09-10: registry, decision facts, generated rollup, report engine, API v2, v1 deleted, hero/pool analysis modules; **B.5b**'s mart half is done — every v2 table carries `hand_uid FixedString(16)` — its `core.*` rebuild stays open) |
| **Next step** | **D.1 Scaffold** — `platform/web/` (Nuxt 4, SPA mode `ssr: false`, TS strict, Pinia, ESLint), `make web`, `make web-check`, CI job, `cors_origins` with `http://localhost:3000`; done when `make web` serves a page that calls `/health` |
| **Branch** | `feat/phase-c-stat-engine` (C.1–C.6 merged into `main` at `274f5db`; C.7/C.8 committed on the branch once the founder says so) |
| **Last updated** | 2026-09-10, session 6 (C.7 analysis modules done and verified on the real data; C.8 wrap-up) |
| **Blockers** | none |
