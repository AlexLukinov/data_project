...
| | |
|---|---|
| **Phase** | **C — Stat engine v2** (C.1–C.6 done 2026-09-09: the v1 chain, compiler and pool report are gone; **B.5b**'s mart half is done — every v2 table carries `hand_uid FixedString(16)` — its `core.*` rebuild stays open) |
| **Next step** | **C.7 Analysis modules** — `analysis/hero/` and `analysis/pool/` (ADR-026): leaks, sessions, cohorts, per-opponent report, presets; `/v1/hero/leaks`, `/v1/pool/cohorts`, `/v1/pool/stats` |
| **Branch** | `feat/phase-c-stat-engine` (C.1–C.6 merged into `main`) |
| **Last updated** | 2026-09-09, session 5 (C.6 cut-over done: v1 dropped, `stats_daily` is the generated rollup, `hand_arrays()` is a macro) |
| **Blockers** | none |
