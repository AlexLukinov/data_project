...
| | |
|---|---|
| **Phase** | **C — Stat engine v2** (C.1–C.3 done 2026-09-09; **B.5b**'s mart half is done — every v2 table carries `hand_uid FixedString(16)` — its `core.*` rebuild stays open) |
| **Next step** | **C.4** — router + service on top of the compiler: `stats/router.py` (rollup vs `player_hands` vs `decisions` by stats AND dimensions), `stats/service.py` (`run_report(ReportRequest, tenant_id) -> ReportResult`, `{value, n}` per cell, buckets for number group-bys, `compare_to`, cache), the 25 v1 compiler tests ported |
| **Branch** | `feat/phase-c-stat-engine` (C.1 and C.2 merged into `main` at `8d8b5ac`) |
| **Last updated** | 2026-09-09, session 5 (C.1–C.3 done and verified) |
| **Blockers** | none — v2 facts and the generated rollup `marts.stats_daily_v2` built over 160/160 partitions beside the untouched v1 chain; `make check` 273 unit tests green |
