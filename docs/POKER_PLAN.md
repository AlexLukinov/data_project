...
| **Next step** | **F.3 Blockers, distribution, metrics** (`poker-core/src/{blockers,distribution,metrics}/`, spec §6–§8, hand-worked tests). F.1 is committed (`e74c148`) and verified locally — its tick waits only for the first green CI `web` run after a push. **F.2 is done** (2026-09-10, uncommitted): fast evaluator, exact + Monte Carlo engine, Worker service; 35-spot treys fixture within tolerance; benchmark far inside every §5.3 target |
| **Branch** | `feat/range-lab` (phase C merged into `main` at `f18049b` on 2026-09-10) |
| **Last updated** | 2026-09-10, session 7 (Range Lab: F.0 report, ADR-027…029, F.1 committed, F.2 done) |
| **Blockers** | none |
