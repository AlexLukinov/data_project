...
| | |
|---|---|
| **Phase** | **C — Stat engine v2** (C.1 and C.2 done 2026-09-09; **B.5b**'s mart half is done — every v2 table carries `hand_uid FixedString(16)` — its `core.*` rebuild stays open) |
| **Next step** | **C.3** — the generator: `scripts/gen_stats.py` renders `marts/stats_daily.sql` from `decisions` + `player_hands`, the definitions seed and the law test for every pair; `make gen` idempotent; CI no-diff |
| **Branch** | `feat/phase-c-stat-engine` (phase B merged into `main` at `1d7f7b9`; C.1 and C.2 are **staged, not committed** — the founder declined the C.1 commit prompt once; ask again) |
| **Last updated** | 2026-09-09, session 5 (C.1 and C.2 done and verified) |
| **Blockers** | none — `marts.decisions` (73,679,949 rows) and `marts.player_hands` (54,562,770) built over 160/160 partitions beside the untouched v1 chain; `make check` 248 unit tests green |
