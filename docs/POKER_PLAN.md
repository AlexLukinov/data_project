...
| **Next step** | **F.9 The 9-step analyzer** (`PredictionGate`, `StepperNav`, steps 1–9 of the spec's §15, Pinia + Dexie autosave, `/v1/analyses`) — the order from the report's §9: F.8 → F.9 → F.10. Done so far: F.1 (`e74c148`), F.2 (`2216e3e`), F.3 (`fd11451`), F.4 = D.1 (`232e096`), D.2 (`3e5cabe`), F.5 (`021f7ae`), F.6 (`951107a`), F.7 (`317fab4`), **F.8 done 2026-09-10, uncommitted** (the pool at a node: `node_filter`, tiers 1 and 2 with `min_n` gating, `PoolDataBadge`, and the GG summary-line parser fix that takes pool showdown cards from 17% to ~100% on a re-parse) — the replayer (`PokerTable`, `HandReplayer`, the replay engine and `nodeKeyAt` in `poker-core`), `POST /v1/hands/{parse,search}`, `GET /v1/pool/hands`, the pages `/hands`, `/hands/[id]`, `/hands/paste` with every panel bound to the current node |
| **Branch** | `feat/range-lab` (phase C merged into `main` at `f18049b` on 2026-09-10); nothing pushed yet |
| **Last updated** | 2026-09-10, session 7 (Range Lab: F.0 report, ADR-027…033, F.1–F.7 + D.2 committed, F.8 done) |
| **Blockers** | none |
