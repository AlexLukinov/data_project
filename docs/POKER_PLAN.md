...
| **Next step** | **D.2 Auth in the SPA** (login/register pages, `stores/auth.ts` with the token in memory and silent refresh on 401, route middleware) — the order from the report's §9: F.4 → D.2 → F.5. Done so far in F: F.1 (`e74c148`), F.2 (`2216e3e`), **F.3 and F.4 (= D.1) done 2026-09-10, uncommitted** — the Range Lab calculator runs at `/lab` |
| **Branch** | `feat/range-lab` (phase C merged into `main` at `f18049b` on 2026-09-10) |
| **Last updated** | 2026-09-10, session 7 (Range Lab: F.0 report, ADR-027…029, F.1 + F.2 committed, F.3 + F.4/D.1 done) |
| **Blockers** | none |
