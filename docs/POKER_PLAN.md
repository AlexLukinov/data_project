...
| | |
|---|---|
| **Phase** | **C — Stat engine v2** (phase B complete 2026-09-09, except the deferred **B.5b** `hand_uid` rebuild, which lands with C.2's table rebuild) |
| **Next step** | **C.1** — the stat registry as data (`stats/registry/*.yaml`, `stats/registry.py`, `stats/ast.py`) |
| **Branch** | `feat/phase-b-module-boundaries` (six phase-B commits on top of `main` `35acf83`; founder decides when to merge) |
| **Last updated** | 2026-09-09, session 4 (B.1–B.8 done and verified; B.5b deferred) |
| **Blockers** | none — ClickHouse up at 4 GB, marts fully rebuilt (160/160 partitions), `make check` 175 unit tests green, `make test-all` 186 passed / 2 skipped in the test environment, real-export parser fingerprint unchanged |
