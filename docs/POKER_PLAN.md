...
| | |
|---|---|
| **Phase** | **C — Stat engine v2** (C.1–C.4 done 2026-09-09; **B.5b**'s mart half is done — every v2 table carries `hand_uid FixedString(16)` — its `core.*` rebuild stays open) |
| **Next step** | **C.5** — API v2 + persistence: `api/routers/{definitions,reports,saved}.py` over `stats.service`, Alembic migration for §2.10 (`saved_filters`, `saved_reports`, `saved_stats`), typed schemas; the old `/v1/stats*` routes become adapters over `stats.service` |
| **Branch** | `feat/phase-c-stat-engine` (C.1–C.3 merged into `main` at `af503bb`) |
| **Last updated** | 2026-09-09, session 5 (C.1–C.4 done and verified) |
| **Blockers** | none — v2 facts and the generated rollup `marts.stats_daily_v2` built over 160/160 partitions beside the untouched v1 chain; `run_report` answers real reports from all three tables; `make check` 305 unit tests green |
