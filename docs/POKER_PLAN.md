...
| **Next step** | **F.8 Pool integration, tiers 1 and 2** (`node_filter()` → AST, `POST /v1/pool/node/{frequencies,showdown-range}`, `PoolDataBadge`, `min_n` gating; **first** answer why only 387,740 of 2,227,803 pool showdown seats carry hole cards) — the order from the report's §9: F.7 → F.8 → F.9. Done so far: F.1 (`e74c148`), F.2 (`2216e3e`), F.3 (`fd11451`), F.4 = D.1 (`232e096`), D.2 (`3e5cabe`), F.5 (`021f7ae`), F.6 (`951107a`), **F.7 done 2026-09-10, uncommitted** — the replayer (`PokerTable`, `HandReplayer`, the replay engine and `nodeKeyAt` in `poker-core`), `POST /v1/hands/{parse,search}`, `GET /v1/pool/hands`, the pages `/hands`, `/hands/[id]`, `/hands/paste` with every panel bound to the current node |
| **Branch** | `feat/range-lab` (phase C merged into `main` at `f18049b` on 2026-09-10); nothing pushed yet |
| **Last updated** | 2026-09-10, session 7 (Range Lab: F.0 report, ADR-027…032, F.1–F.6 + D.2 committed, F.7 done) |
| **Blockers** | none |
