...
| | |
|---|---|
| **Phase** | **B — Module boundaries** (phase A complete 2026-09-09) |
| **Next step** | **B.1** — settings and the ClickHouse client factory out of `api/` |
| **Branch** | `feat/incremental-chain-and-v2-plan` (phase A commits on top of `main` `64ea6cf`; founder decides when to merge) |
| **Last updated** | 2026-09-09, session 3 (audit, A.0–A.9 done and verified) |
| **Blockers** | none — ClickHouse up at 4 GB, marts fully rebuilt (160/160 partitions), `make check` 155 tests green, `make dbt-test` green |
