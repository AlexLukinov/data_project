...
| | |
|---|---|
| **Phase** | **C — Stat engine v2** (C.1–C.5 done 2026-09-09, C.6 parity done; **B.5b**'s mart half is done — every v2 table carries `hand_uid FixedString(16)` — its `core.*` rebuild stays open) |
| **Next step** | **C.6 cut-over** — after the founder confirms the drops: delete the v1 chain, drop the `_v2` suffix, delete `api/queries.py` and `pool_report.py`, make `int_hand_arrays` a macro CTE (exact list in STATUS "Next action") |
| **Branch** | `feat/phase-c-stat-engine` (C.1–C.5 merged into `main` at `cab27e3`) |
| **Last updated** | 2026-09-09, session 5 (C.1–C.5 done; C.6 parity report written, 0 mismatches) |
| **Blockers** | **the C.6 drops need the founder's confirmation** (golden rule 2): `marts.player_hand_flags`, `marts.stats_daily` (v1), `marts.dim_stat_definitions`, the five `intermediate.int_*` v1 tables, then `RENAME marts.stats_daily_v2 → marts.stats_daily` |
