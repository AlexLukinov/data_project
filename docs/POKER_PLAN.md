...
| **Next step** | **F.6 Range library + importers** (Postgres `ranges` + `range_versions`, `/v1/ranges` CRUD with versions and revert, Dexie cache, `NodeKey` lookup, bulk folder import with filename inference and the review table, `packages/poker-importers`, the three-way comparison view over `RangeDiffView`) — the order from the report's §9: F.5 → F.6 → F.7. Done so far: F.1 (`e74c148`), F.2 (`2216e3e`), F.3 (`fd11451`), F.4 = D.1 (`232e096`), D.2 (`3e5cabe`), **F.5 done 2026-09-10, uncommitted** — the metrics panels, the equity graph and the glossary tooltips on `/lab` |
| **Branch** | `feat/range-lab` (phase C merged into `main` at `f18049b` on 2026-09-10); nothing pushed yet |
| **Last updated** | 2026-09-10, session 7 (Range Lab: F.0 report, ADR-027…030, F.1–F.4 + D.2 committed, F.5 done) |
| **Blockers** | none |
