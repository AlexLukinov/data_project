...
| **Next step** | **F.7 Hand replayer** (`PokerTable`, `HandReplayer`, `POST /v1/hands/parse` per ADR-029, `GET /v1/pool/hands`, `filter` on `/v1/hands`; every panel rebinds to the current node) — the order from the report's §9: F.6 → F.7 → F.8. Done so far: F.1 (`e74c148`), F.2 (`2216e3e`), F.3 (`fd11451`), F.4 = D.1 (`232e096`), D.2 (`3e5cabe`), F.5 (`021f7ae`), **F.6 done 2026-09-10, uncommitted** — the range library (`/v1/ranges`, versions, lookup by `NodeKey`), `packages/poker-importers`, the folder import with its review table, the three-way comparison at `/ranges/compare` |
| **Branch** | `feat/range-lab` (phase C merged into `main` at `f18049b` on 2026-09-10); nothing pushed yet |
| **Last updated** | 2026-09-10, session 7 (Range Lab: F.0 report, ADR-027…031, F.1–F.5 + D.2 committed, F.6 done) |
| **Blockers** | none |
